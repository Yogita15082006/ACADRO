...
public interface ExamAiFeedbackRepository extends JpaRepository<ExamAiFeedback, UUID> {
    java.util.Optional<ExamAiFeedback> findByExaminationIdAndStudentIdAndSubjectId(UUID examinationId, UUID studentId, UUID subjectId);
    
    java.util.List<ExamAiFeedback> findByExaminationIdAndStudentId(UUID examinationId, UUID studentId);
